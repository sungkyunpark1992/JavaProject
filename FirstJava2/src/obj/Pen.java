...
public class Pen {
	//클래스 안에 코드
	//attribute(속성, 멤버변수), constructiors(생성자), method(메서드)
	
	//멤버변수 - attribute
	public String color;	//패키지 관계없이 불러올 수 있다.
	int length;
	int price;				//같은 패키지 내에서 불러 올 수 있다. - default
...
//	}
	 ...
	 *  
	 *  ##생성자 만드는 규칙
	 *  1. 생성자 이름은 클래스 이름과 대소문자 까지 정확히 같아야 한다.
	 *  	클래스 이름과 다른 이름으로 만들면 그것은 메서드 취급!
	 *  2. 생성자는 메서드와 달리 반환유형을 적지 않는다; 반환유형이 없다
	...
	public Pen(String init_color, int init_price) {
		color = init_color;
		price = init_price;
		System.out.println("생성자를 이용하여 color, price값 초기화");
	}
	
	public Pen(String color, int length, int price) {
		this.color = color; //this의 의미 : 지금 현재 클래스 내에서
		this.length = length;
		this.price = price;
	}
	...
	// 2. 접금제한자나 반환유형은 오버로딩에 영향을 주지 않는다.
