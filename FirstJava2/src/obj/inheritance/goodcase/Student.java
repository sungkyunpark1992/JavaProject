...
public class Student extends Person{
	 ...
	 *     - this.go() => 자기 자신의 go() 메서드를 참조
	 * 	  > this() :생성자
	 *     - 자기 자신 클래스 내의 다른 생성자를 참조
	 *     - this();  => 기본 생성자를 참조
	 *     - this("hello"); => 자신이 가지고 있는 생성자를 호출. 인자값을 처리하는 생성자
	 *    
	 *   * super
	 *    > 현재 객체의 부모 객체, 한단계 위 부모를 의미함
	 * 	  > super.
	 * 		- 부모의 변수 또는 메서드를 참조
	...
	public Student(String name, int age, String studentId, String major) {
		this(name,age);
		height = 179.8f;	//부모의 멤버 중 protected 멤버 변수
//		weight = 100.0f;		//defaul로 참조가 되지 않는다.
		this.studentId = studentId;
		this.major = major;
	}
	
	public void study() {
		System.out.println(name + "은/는 공부 합니다.");
	}
	...
	public String getDetails() {
		...
		return "[이름: " + name+ "\t나이: " + age + "\t교번: "+studentId+"\t과목: "
		+major+"]";
	}
}
