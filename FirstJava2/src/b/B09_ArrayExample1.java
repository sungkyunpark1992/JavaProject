...
public class B09_ArrayExample1 {
	...
	public static void main(String[] args) {//타입이 String형인 배열
		// 배열
		//같은 타입의 뱐수를 한번 여러개 선언하는 방법
		//배열의 선언 방법
		...
		String[] welcom = new String[] {
				"오늘 뭐 드셨어요?", //인덱스 0번
				"오늘 저녁에 비온대요.. 근데 우산이 없어요..", //인덱스 1
				"졸리죠?" //인덱스 2번
		};
		//배열의 인덱스 : 배열에 저장된 값을 불러올 때에 인덱스 값을 사용
		//			인덱스 값은 시작값이 0 =부터 이다.
		//배열은 참조타입이다.
		System.out.println(welcom[0]);
		System.out.println(welcom[1]);
		System.out.println(welcom[2]);
		System.out.println("welcome 배열의 길이: " +welcom.length);
		System.out.println("numbers 배열의 길이: " +numbers.length);
		System.out.println("win 배열의 길이: " +win.length);
		System.out.println("문자열의 길이: " +welcom[0].length());
		
		//문자열 배열
		char[] s;		//배열 선언
//		char[5] s2;		//배열 선언시 에러; [5] 숫자의 의미는 배열의 길이로 생각하고 쓸 수 없다.
						//배열 선언시 크기를 명시하지 못한다.
		
		s = new char[5];	//배열의 길이로 선언시 초기값이 없는 경우 char는 "\000" 값으로 처기화
...
//		System.out.printf("%s",Arrays.toString(s));
		...
		// ***Enchanced For문(강화된 for문) 연속된 자료형의 경우, 변수;조건;증감을 다 쓰지 않아도 for문을 사용할 수 있게 함
		/*
		 * (형식)
		 ...
		 */
		System.out.println("\n\n강화된 for를 이용한 반복처리");
		for(String str : welcom) {	// welcom은 String[]이기 때문에 String str로 받게 됨.
		...
		System.out.println(Arrays.toString(welcom));
		System.out.println(Arrays.toString(numbers));
		System.out.println(Arrays.toString(win));
		
		//배열 선언시 자동적으로 기본값 초기화
