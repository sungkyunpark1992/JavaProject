...
public class D11_Arrays {
	...
	public static void main(String[] args) {
		...
		double [] argCpy2 = Arrays.copyOf(argOrg, argOrg.length);
		System.out.println("전체 배열 복사");
		for(double d : argCpy2) {
			System.out.print(d+"\t");
		}
		System.out.println();
		
		
		// 1번 인덱스에서 3인덱스까지 복사...(특정 범위 내 부분 복사)
		double[] arrCpy3 = Arrays.copyOfRange(argOrg, 1, 4);
		System.out.println("1번 인덱스에서 3번 인덱스(포함)까지 배열 복사");
		...
		int index = Arrays.binarySearch(scores2, 47);
