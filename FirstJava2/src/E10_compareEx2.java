...
public class E10_compareEx2 {
	...
	public static void main(String[] args) {
		// TreeSet, TreeMap은 이진 검색트리를 구현한 클래스 컬렉션
		//  - 순서가 있기 때문에 정렬이 가능함
		//  - 정렬방법은 Comparator, Comparable 인터페이스를 이용해서 지정
		//  - Comparator는 클래스를 변경하지 않고 사용, 
		//  - Comparable은 클래스에 값을 지정하여 사용해야 한다.
		
		// 이진검색트리란?
		//  - 모든 노드는 최대 두개의 자식 노드를 가질 수 있따.
		//  - 왼쪽 자식 노드의값은 모든 부모 노드의 값보다 작다
		...
		TreeSet<GymMember> gym2 = new TreeSet<>(new Comparator<GymMember>() {
			...
			public int compare(GymMember o1, GymMember o2) {
				...
				//  - o1과 o2의 자리를 바꾸고 싶지 않고 유지하고 싶다면 -1을 리턴
...
class GymMember implements Comparable<GymMember>{
	...
	public int compareTo(GymMember o) {
		// compareTo 메서드는 매개변수 o를 통해서 비교대상을 전달 받고,
		//  - 현재 인스턴스와 o의 자리를 바꾸고 싶은 경우 1을 리턴합니다.
		//  - 현재 인스턴스와 o의 자리를 그대로 유지하고 싶을 때는 -1을 리턴한다.
		//  - 현재 인스턴스와 o가 같은 값임을 나타내고 싶다면 0을 리턴한다.
		int next = o.memNum;
