...
public class C02_method {
	...
	// JAVA에서는 클래스 내부에만 정의할 수 있기 때문에 메서드라는 용어를 사용한다. ( 함수 > 메서드 )
	
	// 메서드의 특징
	// - 어떤 단어 뒤에 ()가 붙은 경우 메서드
	// - 한번 만들어 두면 여러번 사용할 수 있음; 코드의 재사용
	// - 여러 기능들을 묶어서 이름을 붙여 놓은 것이 메서드
	// - 값을 반환할 수 있다.
	
	// 메서드의 형식
	// (접근 제한자) (static;여부) 반환타입 메서드명(매개변수타입 매개변수이름) {
	//		여러가지 기능을 구현
	//	}
	...
	// - void 타입을 지정한 경우 함수의 반환 값이 없음. 즉, return으로 전달할 값이 없다는 의미.
