...
public class C01_Class {
	...
	// - 세상에 존재하는 모든 것든 변수와 메서드로 표현할 수 있다.
	// - 메서드는 객체의 행동 및 기능을 나타내고, 변수는 상태(설정값)를 나타낸다.
	// 클래스(Class)
	// - 객체를 프로그래밍 언어로 표현한 것
	// - 객체의 설계도 
	// - 클래스를 통해 만드는 하나의 실제 객체를 인스턴스라고 부른다.
	// - 클래스의 메서드는 객체의 행동, 클래스의 변수는 객체의 현재 상태를 나타낸다.
	// 클래스 내에 있을때는 메서드라고 한다.
	// - 클래스는 참조형 변수 타입이다.
	
	// JAVA 클래스의 규칙들
	// - public class는 파일 이름과 같아야 한다.
	// - 객체의 설계도
	// - public class는 한 파일에 한개만 존재할 수 있다.(public이 아니면 여러개 존재할 수 있다)
	// - 같은 패키지 내에서는 같은 이름의 클래스는 사용할 수 없다.
	// - 다른 패키지에서 import할 때는 public class만 접근할 수 있다.
	// - public class는 파일의 이름이기 때문에 그 소스 파일 전체의 주제를 알 수 있는 이름이로 짓는 것이 좋다.
