...
class Buyer {
	 ...
	 */
	void buy(Product p) {
		// 구매 못하는 상황 ; 돈이 없을 때, 트렁크 공간이 부족할 때
		if(cash < p.getPrice()) {
	...
	void receipt() {
		...
		viewTrunk();
		//사용 금액과 남은 금액
		System.out.println("사용금액 "+ getTotalPrice(trunk, t_index));
...
class Coffee extends Product {
	// 상속받은 클래스는 생성자에게 반드시 부모 생성자를 호출
	// (호출하지 않으면 기본타입으로 생략되어 있음)
	// 아무것도 적지 않은 생성자엣는 super()가 생략되어 있음
	// product라는 부모 클래스에는 기본 생성자가 없기 때문
...
class SamrtPhone extends Product{

	public SamrtPhone(int price, String name) {
		super(price);
		this.name = name;
	}
